    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-red-50 to-black/5 text-gray-900">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-red-200">Board Card Tracker</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <div className="bg-red-500 border-2 border-white rounded-lg px-4 py-2 shadow-md">
                <div className="text-white text-sm font-bold">Total Boards: {hands.length}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-8">
        ...
        <div className="bg-white border-4 border-black rounded-xl p-6 shadow-2xl">
          <h2 className="text-2xl font-bold mb-4 text-black">Add Board Cards</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-bold text-black mb-2">Board Cards</label>
              <div className="mb-3 text-sm text-black bg-red-50 border-2 border-red-300 rounded-lg p-3">
                <strong className="text-red-700">Format:</strong> Enter one or more board cards, one per line<br/>
                <strong className="text-red-700">Examples:</strong><br/>
                <span className="text-black">• Single: <code className="bg-white px-1 rounded border border-red-300">Qh Jh Th 9s 2c</code></span><br/>
                <span className="text-black">• Multiple: One board per line</span>
              </div>
              <textarea
                placeholder="Enter board cards, one per line..."
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm font-semibold text-black bg-red-100 px-3 py-1 rounded-full border border-red-300">
                {handData.split('\n').filter(line => line.trim()).length} boards ready to add
              </div>
        ...
        {hands.length > 0 ? (
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-lg">
            <h2 className="text-xl font-bold mb-4 text-gray-900">Board History ({hands.length} boards)</h2>
              ...
              {hands.map((hand, index) => (
                <div key={hand.id} className="bg-gray-50 border border-gray-200 rounded-lg p-4 hover:bg-gray-100 transition-colors shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-gray-600 text-sm font-medium">Board #{index + 1}</span>
                    <button
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-12 text-center shadow-lg">
            ...
            <h3 className="text-xl font-medium mb-2 text-gray-900">No boards recorded yet</h3>
            <p className="text-gray-600">Start adding board cards using the form above</p>
          </div>
        )}
      </div>
    </div>
